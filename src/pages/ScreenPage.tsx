...
function Reveal({data}:{data:PublicEventSnapshot}){const r=data.reveal;if(!r)return <ProjectorMessage kicker="THE MOMENT OF TRUTH" title="CHOOSE A TEAM" subtitle="The organiser is preparing the first reveal."/>;if(r.revealStep==='VOTE'){if(r.topVotes.length===0)return <main className="projector reveal-projector"><p className="eyebrow">TEAM {r.teamName.toUpperCase()}</p><h1>NO VOTES.</h1></main>;if(r.topVotes.length===1){const top=r.topVotes[0]!;return <main className="projector reveal-projector"><p className="eyebrow">TEAM {r.teamName.toUpperCase()}</p><h1>YOU ACCUSED…</h1><h2>{top.name}</h2><p>{top.characterName} · {top.votes} votes</p></main>}return <main className="projector reveal-projector"><p className="eyebrow">TEAM {r.teamName.toUpperCase()}</p><h1>YOUR VOTE WAS TIED.</h1><h2>{r.topVotes.map((v)=>v.name).join(' × ')}</h2><p>{r.topVotes[0]!.votes} votes each</p></main>}return <main className={`projector reveal-projector ${r.answer?.correct?'correct':'escaped'}`}><p className="eyebrow">TEAM {r.teamName.toUpperCase()}</p><h1>THE IMPOSTER IS…</h1>{r.answer?<><h2>{r.answer.name}</h2><p>{r.answer.characterName}</p><strong>{r.answer.correct?'IMPOSTER IDENTIFIED ✓':'THE IMPOSTER SURVIVED'}</strong></>:<h2>NOT JOINED</h2>}</main>}
